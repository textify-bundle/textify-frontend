...
const PagesTree: React.FC = () => {
  ...
  const handleSaveNewPage = async () => {
    if (currentProjectId) {
      ...
      try {
        await dispatch(createNewPage({ projectId: currentProjectId, title: pageName })).unwrap()
        setNewPageName("")
  ...
  const handleDeletePage = async () => {
    if (menuPageId) {
      try {
        ...
        alert(`Ошибка при удалении страницы: ${error instanceof Error ? error.message : "Неизвестная ошибка"}`)
      }
    }
  }
    ...
    return (
      <li className="list-item">
        ...
        >
          ...
          {item.type === "link" && item.name !== "Главная" && item.name !== "Корзина" && (
            <IconButton size="small" onClick={() => handleOpenDialog(item.id!)}>
        ...
        <List>
          {tree.map((item: TreeItem, index: number) => (
            <React.Fragment key={item.name}>
              {index === 2 && (
                <div style={{ width: "100%", height: "1px", backgroundColor: "rgba(0, 0, 0, 0.17)" }}></div>
              )}
              <ListItemLink
                item={item}
