...
const ImageEditor = ({ content, nodeId }: { content: MediaContent; nodeId: string }) => {
  ...
  const onResize = (event: unknown, { size }: { size: { width: number; height: number } }) => {
  ...
  const handleMouseUp = () => {
    ...
    localStorage.setItem(`savedImagePosition_${nodeId}`, JSON.stringify(position))
  ...
  return (
    ...
    >
      {!value.url && (
        ...
        >
          {isLoading ? <CircularProgress size={24} color="inherit" /> : "Выбрать изображение"}
        </Button>
      )}
      ...
      {value.url ? (
        <div style={{ position: "relative", width: "100%" }}>
          <IconButton onClick={handleClose} style={{ position: "absolute", top: 0, right: 0, zIndex: 1 }}>
            <CloseIcon />
          </IconButton>
          <Resizable
            width={size.width}
